...
/*
linear algebra functions for the wind assimilation process
*/
...
int permute_lines_wind(double [][NO_OF_CHOSEN_OBSERVATIONS_WIND], int, int);

int inv_gauss_wind(double to_be_inverted[][NO_OF_CHOSEN_OBSERVATIONS_WIND], double inv[][NO_OF_CHOSEN_OBSERVATIONS_WIND])
{
	/*
	...
	*/
	...
	double factor;
	for (int i = 0; i < NO_OF_CHOSEN_OBSERVATIONS_WIND - 1; ++i)
	{
		/*
		...
		*/
		...
		{
			permute_lines_wind(to_be_inverted, i, permute_index_counter);
			permute_lines_wind(inv, i, permute_index_counter);
		}
	...
	for (int j = 0; j < NO_OF_CHOSEN_OBSERVATIONS_WIND; ++j)
	{
		inv[NO_OF_CHOSEN_OBSERVATIONS_WIND - 1][j] = inv[NO_OF_CHOSEN_OBSERVATIONS_WIND - 1][j]/to_be_inverted[NO_OF_CHOSEN_OBSERVATIONS_WIND - 1][NO_OF_CHOSEN_OBSERVATIONS_WIND - 1];
	}
	to_be_inverted[NO_OF_CHOSEN_OBSERVATIONS_WIND - 1][NO_OF_CHOSEN_OBSERVATIONS_WIND - 1] = 1;
	
	/*
	Gaussian upwards
	----------------
	*/
	for (int i = NO_OF_CHOSEN_OBSERVATIONS_WIND - 1; i >= 1; --i)
	{
		#pragma omp parallel for
		for (int j = i - 1; j >= 0; --j)
		{
...
int permute_lines_wind(double matrix[][NO_OF_CHOSEN_OBSERVATIONS_WIND], int line_a, int line_b)
{
	/*
	Permutes line_a with line_b of matrix.
	*/
	double line_a_pre[NO_OF_CHOSEN_OBSERVATIONS_WIND];
	#pragma omp parallel for
	for (int i = 0; i < NO_OF_CHOSEN_OBSERVATIONS_WIND; ++i)
	{

...
\begin{center}
\begin{align}
x_i &= x_{B, i} + \sum_{j = 1}^N\left[\sum_{k = 1}^N\left(\overleftrightarrow{H}\overleftrightarrow{B}\overleftrightarrow{H}^T + \overleftrightarrow{R}\right)^{-1}_{k, j}\overleftrightarrow{B}_{i, i}\overleftrightarrow{H}_{k, i}\right]\left(y_j - h_j\right).
\end{align}
\end{center}
}}
\end{center}
